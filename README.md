...
- "Reset layout" button to snap boards back into a tidy grid.
...
- Upload your own wallpaper image — shown full-bleed, full resolution, with only a light tint for text legibility.
...
4. Select the unpacked extension folder, for example `E:\Code\Chrome Extension Files\MintList-New-Tab-1.4.1-20260711-163538`.
5. Open a new tab.
...
Boards live on a freeform canvas — drag a board by its header to place it anywhere. The canvas has no visible scrollbar; it only grows as far as your boards actually extend. Click **Reset layout** (bottom-right of the board area) at any time to snap every board on the current page back into a neat grid.
